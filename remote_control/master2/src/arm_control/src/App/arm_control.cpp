...
arx_arm::arx_arm(int CONTROL_MODE)
{
    ...
    if (play.modifyLinkMass(model_path, model_path, 0.640)) { //默认单位kg 夹爪质量 0.381 0.581  640
...
void arx_arm::motor_control(ecat::EcatBase ecat_base)
{
    ...
    {               
                    CAN_Handlej.Send_moto_Cmd1(ecat_base,1, 0, 12, 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_moto_Cmd1(ecat_base,2, 0, 12, 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_moto_Cmd1(ecat_base,4, 0, 12, 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_moto_Cmd2(ecat_base,5, 0, 1 , 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_moto_Cmd2(ecat_base,6, 0, 1 , 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_moto_Cmd2(ecat_base,7, 0, 1 , 0, 0, 0);usleep(200); 
                    ROS_WARN("safe mode!!!!!!!!!");
...
void arx_arm::init_step(ecat::EcatBase ecat_base)
{
   ...
   if(temp_init>2)
   {
            ...
            CAN_Handlej.Send_moto_Cmd1(ecat_base,1, init_kp, init_kd, target_pos[0], 0,  0); usleep(200);         
            CAN_Handlej.Send_moto_Cmd1(ecat_base,2, init_kp, init_kd, target_pos[1], 0,  0); usleep(200);
            CAN_Handlej.Send_moto_Cmd1(ecat_base,4, init_kp, init_kd, target_pos[2], 0,  0); usleep(200);
